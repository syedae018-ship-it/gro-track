  ...
  return (
    <div className="flex flex-col gap-6 w-full pb-12">
      ...
      <div className="flex flex-col gap-4">
        <Link href="/dashboard/clients" className="flex items-center gap-2 text-sm text-white/40 hover:text-white transition-colors w-fit">
          <ArrowLeft className="w-4 h-4" /> Back to Clients
        </Link>
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-white/10 to-white/5 border border-white/10 flex items-center justify-center text-3xl font-syne font-bold text-white shadow-xl shadow-violet-500/10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-[#121214] border border-emerald-500/20 rounded-2xl p-5 relative overflow-hidden">
        ...
        <div className="bg-[#121214] border border-white/5 rounded-2xl p-5">
          <span className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider font-bold text-white/40 mb-1">
        ...
        <div className="bg-[#121214] border border-white/5 rounded-2xl p-5">
          ...
          <span className="text-2xl font-bold text-white">{uniqueEmployees.length}</span>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          ...
          <div className="bg-[#121214] border border-white/5 rounded-2xl p-6">
            <h2 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-yellow-400 shadow-[0_0_10px_rgba(250,204,21,0.5)]" />
              Active Tasks
            </h2>
            <div className="flex flex-col gap-3">
              {activeTasks.length === 0 ? (
                <div className="text-sm text-white/30 py-4 text-center border border-dashed border-white/5 rounded-xl">No active tasks</div>
              ) : (
          ...
          <div className="bg-[#121214] border border-white/5 rounded-2xl p-6">
            ...
            <div className="flex flex-col gap-3">
              {completedTasks.length === 0 ? (
                <div className="text-sm text-white/30 py-4 text-center border border-dashed border-white/5 rounded-xl">No completed tasks yet</div>
              ) : (
                completedTasks.slice(0, 10).map((task: any) => (
                  <div key={task.id} className="bg-white/5 border border-white/5 rounded-xl p-4 flex items-center justify-between opacity-80">
                    <div>
                      <h3 className="text-sm font-semibold text-white/90 mb-1 line-through decoration-white/20">{task.title}</h3>
                      <div className="flex items-center gap-3 text-[10px] text-white/40 font-medium">
                        {task.assignee && (
          ...
          <div className="bg-[#121214] border border-white/5 rounded-2xl p-6">
            ...
            <div className="flex flex-col gap-3">
              {uniqueEmployees.length === 0 ? (
                <div className="text-sm text-white/30">No employees assigned</div>
              ) : (
                uniqueEmployees.map((emp: any, idx: number) => (
                  <div key={idx} className="flex items-center gap-3 bg-white/5 p-2 rounded-xl border border-white/5">
                    <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-violet-500/20 to-blue-500/20 border border-white/10 flex items-center justify-center text-xs font-bold text-white">
                      {emp.full_name.charAt(0)}
                    </div>
                    <span className="text-sm font-medium text-white/90">{emp.full_name}</span>
                  </div>
                ))
              )}
            </div>
