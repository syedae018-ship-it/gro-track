  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {clients?.length === 0 && (
          <div className="col-span-full p-12 text-center text-muted border border-white/5 rounded-2xl bg-white/5 border-dashed">
        ...
        {clients?.map((client) => {
          ...
          return (
            <Link key={client.id} href={`/dashboard/clients/${client.id}`} className="block">
              <div className="bg-[#121214] border border-white/5 rounded-2xl p-6 hover:border-violet-500/30 transition-all group relative overflow-hidden h-full flex flex-col hover:shadow-xl hover:shadow-violet-500/10 hover:-translate-y-1">
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-white/10 to-white/5 border border-white/10 flex items-center justify-center text-xl font-syne font-bold text-white shadow-lg">
                ...
                <div className="grid grid-cols-2 gap-3 mb-6 flex-1">
                  <div className="bg-white/5 border border-white/5 rounded-xl p-3">
                    ...
                    <span className="text-lg font-bold text-white">{activeTasks.length}</span>
                  </div>
                  <div className="bg-white/5 border border-white/5 rounded-xl p-3">
                    <span className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider font-bold text-white/40 mb-1">
                      <CheckCircle2 className="w-3 h-3 text-emerald-400" /> Completed
                    </span>
                    <span className="text-lg font-bold text-white">{completedTasks.length}</span>
                  </div>
                </div>
