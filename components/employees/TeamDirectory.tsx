...
import {
 ...
 UserCircle2, Banknote, Link2, Clock
} from "lucide-react"
...
export function TeamDirectory({ employees, tasks, attendance }: TeamDirectoryProps) {
  ...
  // Build map of employee ID -> total worked hours
  const employeeWorkedHours = useMemo(() => {
    ...
    if (attendance) {
      attendance.forEach(log => {
        ...
        if (workedHoursFilter === 'week') {
          ...
          include = log.check_in && new Date(log.check_in) >= cutoff
        } else if (workedHoursFilter === 'month') {
          const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1)
          include = log.check_in && new Date(log.check_in) >= startOfMonth
        ...
        if (include) {
          map[log.employee_id] = (map[log.employee_id] || 0) + (log.duration_minutes || 0)
        }
      })
    }
    const hoursMap: Record<string, number> = {}
    Object.entries(map).forEach(([empId, mins]) => {
      hoursMap[empId] = Number((mins / 60).toFixed(1))
    })
    return hoursMap
  }, [attendance, workedHoursFilter])
 ...
 <>
 {/* ── Filters ── */}
 <div className="flex flex-col md:flex-row gap-3 bg-card/[0.02] border border-border rounded-xl p-3">
 ...
 className="w-full bg-card border border-border rounded-xl pl-9 pr-4 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-violet-500/40 transition-all"
 />
 </div>
 <div className="flex flex-wrap gap-2">
 <select value={filterRole} onChange={e => setFilterRole(e.target.value)} className="bg-card border border-border rounded-xl px-3 py-2 text-sm text-foreground focus:outline-none focus:border-violet-500/40 transition-all">
 <option value="">All Roles</option>
 {Object.entries(ROLE_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
 </select>
 <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="bg-card border border-border rounded-xl px-3 py-2 text-sm text-foreground focus:outline-none focus:border-violet-500/40 transition-all">
 <option value="">All Status</option>
 <option value="active">Active</option>
 <option value="inactive">Inactive</option>
 </select>
 <select value={workedHoursFilter} onChange={e => setWorkedHoursFilter(e.target.value as any)} className="bg-card border border-border rounded-xl px-3 py-2 text-sm text-foreground focus:outline-none focus:border-violet-500/40 transition-all">
   <option value="week">Hours: This Week</option>
   <option value="month">Hours: This Month</option>
   <option value="30">Hours: 30 Days</option>
   <option value="60">Hours: 60 Days</option>
   <option value="all">Hours: All Time</option>
 </select>
 <select value={filterClient} onChange={e => setFilterClient(e.target.value)} className="bg-card border border-border rounded-xl px-3 py-2 text-sm text-foreground focus:outline-none focus:border-violet-500/40 transition-all">
 ...
 className="bg-card border border-border rounded-2xl overflow-hidden hover:border-border hover:shadow-lg hover:shadow-black/30 transition-all group"
 >
 {/* Card Header */}
 ...
 <div>
 <p className="font-bold text-foreground text-[15px] leading-tight truncate">{emp.full_name}</p>
 <p className="text-[11px] text-violet-300/80 mt-0.5 font-medium">
 {emp.specialization || ROLE_LABELS[emp.role] || emp.role}
 </p>
 </div>
 {(() => {
   const statusLabel = emp.current_status === "checked_in" 
     ? "Active" 
     : emp.current_status === "break" 
       ? "On Break" 
       : "Duty Off"
   
   const statusColor = emp.current_status === "checked_in"
   ...
   return (
     <span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border shrink-0 ${statusColor}`}>
       {emp.status_emoji || "●"} {statusLabel}
     </span>
   )
 })()}
 </div>
 ...
 {emp.phone ? (
 <a href={`tel:${emp.phone}`} className="flex items-center gap-2 text-[11px] text-muted-foreground hover:text-foreground transition-colors">
 ...
 {emp.email ? (
 <a href={`mailto:${emp.email}`} className="flex items-center gap-2 text-[11px] text-muted-foreground hover:text-foreground transition-colors truncate">
 ...
 <span className="text-muted-foreground font-normal"> · {emp.upi_id}</span>
 )}
 </span>
 </div>
 </div>
 ...
 {clients.length > 4 && (
 <span className="text-[10px] text-muted-foreground px-1.5 py-0.5">+{clients.length - 4}</span>
 )}
 </div>
 </div>
 )}
 ...
 <div className="flex flex-col gap-1 text-[10px] text-muted-foreground">
   ...
   <div className="flex items-center gap-1.5">
     ...
     <span>{employeeWorkedHours[emp.id] || 0} worked hours</span>
   </div>
 </div>
 <div className="flex items-center gap-1">
 <a
 href={`/dashboard/analytics`}
 className="w-7 h-7 rounded-lg bg-muted hover:bg-violet-500/15 hover:text-violet-400 flex items-center justify-center text-muted-foreground transition-all"
 ...
 className="flex items-center gap-1 text-[10px] font-semibold text-muted-foreground hover:text-violet-300 bg-muted hover:bg-violet-500/10 px-2.5 py-1.5 rounded-lg transition-all"
 >
 ...
 animate={{ opacity: 1 }}
 exit={{ opacity: 0 }}
 className="fixed inset-0 z-50 bg-muted flex items-start justify-center p-4 pt-8 overflow-y-auto"
 ...
 className="bg-card border border-border rounded-3xl w-full max-w-2xl shadow-2xl shadow-black/60 overflow-hidden mb-8"
 >
 {/* Modal Header */}
 {(() => {
 ...
 <div>
 <h2 className="text-2xl font-syne font-bold text-foreground">{emp.full_name}</h2>
 ...
 className="w-8 h-8 rounded-xl bg-muted hover:bg-muted flex items-center justify-center text-muted-foreground hover:text-foreground transition-all shrink-0"
 >
 <X className="w-4 h-4" />
 </button>
 </div>
 ...
 {/* Contact Info */}
 <div className="bg-card/[0.03] border border-border rounded-2xl p-4 flex flex-col gap-3">
 <h3 className="text-[10px] uppercase tracking-wider font-bold text-muted-foreground flex items-center gap-2">
 ...
 {/* Assigned Clients */}
 <div className="bg-card/[0.03] border border-border rounded-2xl p-4">
 <h3 className="text-[10px] uppercase tracking-wider font-bold text-muted-foreground mb-3 flex items-center gap-2">
 ...
 {/* Notes */}
 {emp.notes && (
 <div className="mt-5 bg-card/[0.03] border border-border rounded-2xl p-4">
 <h3 className="text-[10px] uppercase tracking-wider font-bold text-muted-foreground mb-2">Admin Notes</h3>
 <p className="text-sm text-muted-foreground leading-relaxed">{emp.notes}</p>
 </div>
 )}
 ...
 {recentTasks.map(t => (
 <div key={t.id} className="flex items-center justify-between gap-3 text-xs py-1.5 border-b border-border last:border-0">
...
function InfoRow({
 ...
 {value ? (
 href ? (
 <a href={href} className="text-xs text-muted-foreground hover:text-foreground transition-colors truncate block">{value}</a>
 ) : copyable ? (
 <button onClick={handleCopy} className="text-xs text-muted-foreground hover:text-foreground transition-colors text-left truncate w-full">
