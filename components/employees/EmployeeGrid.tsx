    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
      {enriched.map((emp, i) => (
        <motion.div
          ...
          className="bg-[#111115] border border-white/5 rounded-2xl p-5 flex flex-col gap-4 hover:border-white/10 hover:shadow-lg hover:shadow-black/30 transition-all group"
        >
          {/* Header */}
          <div className="flex items-start gap-3">
            <Avatar initials={emp.full_name} src={emp.avatar_url} size="md" />
            <div className="flex-1 min-w-0">
              <p className="font-bold text-white text-[15px] leading-tight">{emp.full_name}</p>
              <p className="text-[11px] text-white/40 mt-0.5">{ROLE_LABELS[emp.role] || emp.role}</p>
            </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-white/[0.03] border border-white/5 rounded-xl p-2.5 flex flex-col items-center text-center">
              <ListTodo className="w-3.5 h-3.5 text-yellow-400 mb-1" />
              <span className="text-base font-bold text-white">{emp.activeCount}</span>
              <span className="text-[9px] text-white/30 uppercase tracking-wider">Active</span>
            </div>
            <div className="bg-white/[0.03] border border-white/5 rounded-xl p-2.5 flex flex-col items-center text-center">
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 mb-1" />
              <span className="text-base font-bold text-white">{emp.completedCount}</span>
              <span className="text-[9px] text-white/30 uppercase tracking-wider">Done</span>
            </div>
            <div className="bg-white/[0.03] border border-white/5 rounded-xl p-2.5 flex flex-col items-center text-center">
              <TrendingUp className="w-3.5 h-3.5 text-violet-400 mb-1" />
              <span className="text-base font-bold text-white">{emp.productivity}%</span>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="bg-amber-500/5 border border-amber-500/10 rounded-xl p-3">
              <span className="text-[9px] uppercase tracking-wider font-bold text-amber-500/60 block mb-0.5">Pending Payout</span>
              <span className="text-sm font-black text-amber-400">₹{emp.pendingPayouts.toLocaleString('en-IN')}</span>
            </div>
          </div>
          ...
          {emp.clients.length > 0 && (
            <div>
              <p className="text-[9px] uppercase tracking-wider font-bold text-white/25 mb-2 flex items-center gap-1">
