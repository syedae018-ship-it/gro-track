...
export function PaymentsDashboard({ payouts }: PaymentsDashboardProps) {
 ...
 {/* Status pills */}
 <div className="flex gap-1.5 bg-card/[0.03] border border-border p-1 rounded-xl overflow-x-auto mobile-scroll-x w-full sm:w-auto">
 ...
 : s === "pending" ? "bg-amber-500/20 text-muted-foregroundmber-400 border border-amber-500/30"
 ...
 {/* Date range */}
 <div className="flex gap-1 bg-card/[0.03] border border-border p-1 rounded-xl">
 ...
 className="bg-amber-500/5 border border-amber-500/20 rounded-2xl p-5 relative overflow-hidden">
 ...
 <Clock className="w-4 h-4 text-muted-foregroundmber-400" />
 </div>
 <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foregroundmber-500/60">Pending</span>
 </div>
 <p className="text-2xl font-black text-muted-foregroundmber-400">{fmtAmount(totalPending)}</p>
 <p className="text-[10px] text-muted-foregroundmber-500/40 mt-1">{pendingCount} payout{pendingCount !== 1 ? "s" : ""} outstanding</p>
 </div>
 </motion.div>

 <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
 className="bg-card/[0.03] border border-border rounded-2xl p-5">
 <div className="flex items-center gap-2 mb-3">
 <div className="w-8 h-8 rounded-xl bg-violet-500/15 border border-violet-500/20 flex items-center justify-center">
 <TrendingUp className="w-4 h-4 text-violet-400" />
 </div>
 <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Total Volume</span>
 </div>
 <p className="text-2xl font-black text-foreground">{fmtAmount(totalPaid + totalPending)}</p>
 <p className="text-[10px] text-muted-foreground mt-1">{filtered.length} total transactions</p>
 </motion.div>
 ...
 <Banknote className="w-4 h-4 text-muted-foregroundlue-400" />
 </div>
 <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Avg Payout</span>
 </div>
 ...
 {/* Table Header */}
 <div className="grid grid-cols-[2fr_2fr_1fr_1.5fr_1.5fr_1fr_1fr] px-4 py-3 border-b border-border gap-4 min-w-[700px]">
 ...
 <p className="text-sm font-semibold text-muted-foreground">No payout transactions yet.</p>
 <p className="text-xs text-muted-foreground">Transactions will appear here after employees request payouts.</p>
 </div>
 ) : (
 ...
 className="w-full grid grid-cols-[2fr_2fr_1fr_1.5fr_1.5fr_1fr_1fr] px-4 py-3.5 gap-4 hover:bg-card/[0.03] transition-colors text-left group items-center min-w-[700px]"
 >
 {/* Employee */}
 <div className="flex items-center gap-2.5 min-w-0">
 <Avatar initials={employeeName} src={payout.employee?.avatar_url} size="sm" className="w-7 h-7 rounded-lg" />
 <div className="min-w-0">
 <p className="text-xs font-semibold text-foreground truncate">{employeeName}</p>
 <p className="text-[9px] text-muted-foreground">{ROLE_LABELS[payout.employee?.role] || "—"}</p>
 </div>
 </div>
 ...
 {clients.length > 2 && (
 <span className="text-[9px] text-muted-foreground">+{clients.length - 2}</span>
 )}
 </>
 )}
 </div>
 ...
 {/* Requested */}
 <span className="text-xs text-muted-foreground">{fmtDate(payout.requested_at)}</span>

 {/* Paid On */}
 <span className={`text-xs ${paid ? "text-muted-foreground" : "text-muted-foreground"}`}>{fmtDate(payout.paid_at)}</span>

 {/* Amount */}
 <span className={`text-sm font-black ${paid ? "text-emerald-400" : "text-muted-foregroundmber-400"}`}>
 ...
 <ChevronRight className="w-3 h-3 text-muted-foreground group-hover:text-muted-foreground transition-colors ml-auto" />
 </div>
 </motion.button>
 )
 })}
 ...
 animate={{ opacity: 1 }}
 exit={{ opacity: 0 }}
 className="fixed inset-0 z-50 bg-muted flex items-end sm:items-start justify-center sm:p-4 sm:pt-10 overflow-y-auto"
 ...
 className="bg-card border border-border rounded-3xl w-full max-w-xl shadow-2xl shadow-black/60 overflow-hidden mb-8"
 >
 {(() => {
 ...
 <div>
 <h2 className="text-xl font-syne font-bold text-foreground">{p.employee?.full_name || "Unknown"}</h2>
 <div className="flex items-center gap-2 mt-0.5">
 <span className="text-xs text-muted-foreground">{ROLE_LABELS[p.employee?.role] || "—"}</span>
 ...
 <p className="text-[10px] uppercase tracking-widest font-bold mb-1 ${paid ? 'text-emerald-500/50' : 'text-muted-foregroundmber-500/50'}">Payout Amount</p>
 <p className={`text-4xl font-black ${paid ? "text-emerald-400" : "text-muted-foregroundmber-400"}`}>
 ...
 <div className="bg-card/[0.03] border border-border rounded-xl p-3">
 <p className="text-[9px] uppercase tracking-wider font-bold text-muted-foreground mb-1 flex items-center gap-1">
 <Calendar className="w-3 h-3" /> Requested
 </p>
 <p className="text-xs font-semibold text-foreground">{fmtDate(p.requested_at)}</p>
 </div>
 <div className="bg-card/[0.03] border border-border rounded-xl p-3">
 <p className="text-[9px] uppercase tracking-wider font-bold text-muted-foreground mb-1 flex items-center gap-1">
 <CheckCircle2 className="w-3 h-3" /> Paid On
 </p>
 <p className={`text-xs font-semibold ${paid ? "text-foreground" : "text-muted-foreground"}`}>
 ...
 <p className="text-xs font-semibold text-foreground">{taskCount} task{taskCount !== 1 ? "s" : ""}</p>
 </div>
 <div className="bg-card/[0.03] border border-border rounded-xl p-3">
 <p className="text-[9px] uppercase tracking-wider font-bold text-muted-foreground mb-1 flex items-center gap-1">
 ...
 <div className="min-w-0">
 <p className="text-xs font-medium text-muted-foreground truncate">{task.title}</p>
