  ...
  return (
    <div className="flex flex-col gap-6 w-full relative">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-syne font-bold text-white">My Earnings</h1>
          <p className="text-sm text-white/40 mt-0.5">Track your task payments and payout history.</p>
        </div>
        ...
        {[
          ...
          return (
            <Wrapper
              ...
              className={`p-5 rounded-2xl border ${card.border} ${card.bg} flex flex-col gap-3 relative overflow-hidden group text-left ${card.interactive ? 'cursor-pointer hover:shadow-[0_0_30px_rgba(250,204,21,0.1)]' : ''} transition-shadow duration-300`}
            >
              <div className={`w-8 h-8 rounded-full ${card.bg} border ${card.border} flex items-center justify-center`}>
                <Icon className={`w-4 h-4 ${card.color}`} />
              </div>
              <div>
                <p className="text-[11px] font-semibold text-white/30 uppercase tracking-widest mb-1">{card.label}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
          <div className="glass-card border border-border rounded-2xl overflow-hidden shadow-purple-bloom">
            <div className="px-5 py-4 border-b border-border bg-white/[0.01]">
                ...
                filteredTasks.map((task) => (
                  ...
                  >
                    <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center shrink-0">
                      <CheckCircle2 className="w-4 h-4 text-white/40 group-hover:text-emerald-400 transition-colors" />
                    </div>
                    ...
                    <div className="flex flex-col items-end shrink-0 gap-1.5">
                      <p className="text-sm font-bold text-emerald-400 drop-shadow-[0_0_6px_rgba(52,211,153,0.4)]">
                      ...
                      <span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md border ${
                        ...
                        task.payout_status === 'payout_requested' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' :
        ...
        <div className="flex flex-col gap-4">
          <div className="glass-card border border-border rounded-2xl overflow-hidden shadow-purple-bloom">
            <div className="px-5 py-4 border-b border-border bg-white/[0.01]">
              <h2 className="font-semibold text-white text-sm">Recent Payouts</h2>
              <p className="text-[11px] text-white/40 mt-0.5">Your requested and paid earnings.</p>
            </div>
            <div className="divide-y divide-white/5 max-h-[500px] overflow-y-auto">
              {filteredPayouts.length === 0 ? (
                <div className="py-12 flex flex-col items-center justify-center text-white/30 px-6 text-center">
                ...
                filteredPayouts.map((payout) => (
                  <div key={payout.id} className="px-5 py-4 flex flex-col gap-3 hover:bg-white/[0.02] transition-colors">
          ...
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-black/60 backdrop-blur-md">
            ...
            >
              <div className="p-6 border-b border-white/5">
                ...
                <p className="text-[11px] text-white/40 mt-1">Review your unpaid tasks before submitting.</p>
              </div>

              <div className="p-6 bg-black/20 flex flex-col gap-4 max-h-[40vh] overflow-y-auto">
                {unpaidTasks.length === 0 ? (
                  <div className="text-center py-6">
                    ...
                    <p className="text-xs text-white/40 mt-1">No pending tasks to request payout for.</p>
                  </div>
                ) : (
                  <>
                    <div className="flex items-center justify-between p-4 bg-yellow-500/5 border border-yellow-500/10 rounded-xl">
                      ...
                      <div className="text-right">
                        <p className="text-[10px] uppercase tracking-wider font-semibold text-white/30">Tasks</p>
                        <p className="text-lg font-bold text-white/80 mt-0.5">{selectedTaskIds.length}</p>
                      </div>
                    </div>

                    <div className="space-y-2">
                      <div className="flex items-center justify-between pl-1 mb-2">
                        <p className="text-[10px] font-semibold text-white/30 uppercase tracking-wider">Select Tasks Included</p>
                        <button 
                          onClick={() => setSelectedTaskIds(selectedTaskIds.length === unpaidTasks.length ? [] : unpaidTasks.map(t => t.id))}
                          className="text-[10px] font-semibold text-white/40 hover:text-white/80 transition-colors"
                        >
                      ...
                      {unpaidTasks.map(t => {
                        ...
                        return (
                          <div 
                            ...
                            className={`flex items-center gap-3 p-3 border rounded-lg cursor-pointer transition-all ${isSelected ? 'bg-white/[0.05] border-white/20' : 'bg-black/20 border-white/5 opacity-60 hover:opacity-100'}`}
                          >
                            <div className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-colors ${isSelected ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-white/20'}`}>
                              {isSelected && <CheckCircle2 className="w-3 h-3" />}
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className={`text-xs font-medium truncate pr-4 ${isSelected ? 'text-white/90' : 'text-white/60'}`}>{t.title}</p>
                            </div>
                            <p className={`text-xs font-bold shrink-0 ${isSelected ? 'text-emerald-400' : 'text-white/40'}`}>{formatINR(safeNum(t.payment_amount))}</p>
                          </div>
                        )
                      })}
                    </div>
              ...
              <div className="p-5 border-t border-white/5 flex flex-col gap-3">
                <div className="flex gap-3">
                  <button
                    ...
                    className="flex-1 py-3 rounded-xl text-xs font-bold text-black bg-white hover:bg-gray-200 disabled:opacity-50 disabled:hover:bg-white transition-all flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(255,255,255,0.1)] hover:shadow-[0_0_25px_rgba(255,255,255,0.2)]"
                  >
                  ...
                  <button
                    ...
                    disabled={selectedTaskIds.length === 0 || isRequesting}
                    className="flex-1 py-3 rounded-xl text-xs font-bold text-[#25D366] bg-[#25D366]/10 border border-[#25D366]/20 hover:bg-[#25D366]/20 disabled:opacity-50 transition-all flex items-center justify-center gap-1.5 shadow-[0_0_20px_rgba(37,211,102,0.1)] hover:shadow-[0_0_25px_rgba(37,211,102,0.15)]"
                  >
                ...
                <button
                  ...
                  className="w-full mt-1 py-2 rounded-xl text-xs font-semibold text-white/40 hover:text-white/80 transition-colors"
                >
                  Cancel
                </button>
              </div>
