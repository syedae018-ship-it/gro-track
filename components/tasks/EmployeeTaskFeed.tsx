...
import { useStreakSync } from "@/hooks/use-streak-sync"
...
export const EmployeeTaskFeed = memo(function EmployeeTaskFeed({ tasks: initialTasks, currentUserId, userName }: EmployeeTaskFeedProps) {
 ...
 return allCompleted.filter(t => t.completed_at && new Date(t.completed_at) >= cutoff)
  ...
  const { data: streakData } = useStreakData(currentUserId)
  const { recordTaskCompletion } = useStreakSync(currentUserId)

  // Work streak — timezone-aware, computed dynamically to check if broken
  const displayStreak = useMemo(() => {
    if (!streakData) return 0
    const today = new Date().toLocaleDateString("en-CA")
    const lastDate = streakData.last_completed_date
    if (!lastDate) return 0
    if (lastDate === today) return streakData.current_streak
    ...
    if (diff === 1) return streakData.current_streak
 ...
 const handleTaskUpdate = useCallback((updatedTask: any) => {
   setTasks(prev => {
     ...
     if (oldTask && oldTask.status !== updatedTask.status && ["completed", "approved"].includes(updatedTask.status)) {
       // Only count as completion if it transitioned into completed state
 ...
 {
 id: "streak",
 label: `Work Streak (Best: 🏆 ${streakData?.longest_streak || 0}d)`,
 value: displayStreak > 0 ? `🔥 ${displayStreak}d` : "—",
 icon: Flame,
 color: "text-orange-400",
 bg: "bg-orange-500/8",
 border: "border-orange-500/15",
 glow: displayStreak > 0 ? "shadow-orange-500/10" : "",
 },
 {
 id: "productivity",
 label: "Productivity",
 ...
 : "text-muted-foreground bg-muted border-border hover:text-muted-foreground"
 }`}
 >
 ...
 <PersonalTasks onTaskCompleted={recordTaskCompletion} />

 {/* ── PENDING ── */}
 <section>
 <div className="flex items-center gap-2.5 mb-3">
 <span className="w-2 h-2 rounded-full bg-red-500 shadow-sm animate-pulse" />
 <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">Pending</h2>
 ...
 initial={{ opacity: 0 }}
 animate={{ opacity: 1 }}
 className="py-12 rounded-2xl border border-dashed border-border flex flex-col items-center justify-center text-center"
 >
 ...
 <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">Completed</h2>
 ...
 className="fixed right-5 sm:right-8 w-[52px] h-[52px] rounded-2xl bg-gradient-to-br from-violet-600 to-blue-600 text-foreground shadow-2xl shadow-violet-500/25 flex items-center justify-center z-40 hover:shadow-violet-500/40 transition-shadow active:scale-95"
 >
