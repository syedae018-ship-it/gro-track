...
export function ProductivityAnalytics({ tasks }: ProductivityAnalyticsProps) {
  ...
  return (
    <div className="bg-[#0d0d12] border border-white/5 rounded-2xl p-5 flex flex-col gap-5">
      <div className="flex items-center gap-2">
        <TrendingUp className="w-4 h-4 text-violet-400" />
        <h2 className="text-sm font-semibold text-white">Productivity Analytics</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {statCards.map((card, i) => (
          ...
          >
            ...
            {card.sub && <p className="text-[10px] text-white/25 truncate mt-0.5">{card.sub}</p>}
            <p className="text-[10px] text-white/30 uppercase tracking-wide mt-1">{card.label}</p>
          </motion.div>
        ))}
      </div>
      ...
      {/* Weekly bar chart */}
      <div>
        <p className="text-xs text-white/30 mb-3 uppercase tracking-wider">Weekly Output</p>
        <div className="h-32">
