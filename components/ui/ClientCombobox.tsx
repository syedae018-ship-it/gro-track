...
export function ClientCombobox({
  ...
  const [selected, setSelected] = useState<ClientSelection>(selectedValue || EMPTY)
 ...
 className={`w-full flex items-center justify-between gap-2 bg-muted border rounded-xl px-3.5 py-2.5 text-sm transition-all focus:outline-none ${
 isOpen
 ...
 <>
 {isCustom ? (
 <PenLine className="w-3.5 h-3.5 text-muted-foregroundmber-400 shrink-0" />
 ) : (
 ...
 <span className={`truncate font-medium ${isCustom ? "text-muted-foregroundmber-300" : "text-foreground"}`}>
 {selected.displayName}
 </span>
 {isCustom && (
 <span className="text-[9px] uppercase tracking-wider font-bold text-muted-foregroundmber-500/60 bg-amber-500/10 border border-amber-500/20 px-1.5 py-0.5 rounded-full shrink-0">
 Custom
 </span>
 )}
 </>
 ...
 type="button"
 onClick={clear}
 className="absolute right-8 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-muted-foreground text-xs transition-colors"
 >
 ✕
 </button>
 )}
 ...
 className="absolute top-full left-0 right-0 mt-1.5 z-50 bg-card border border-border rounded-2xl shadow-2xl overflow-hidden"
 >
 {/* Search Input */}
 ...
 className="w-full bg-muted border border-border rounded-xl pl-8 pr-3 py-2 text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-violet-500/40 transition-all"
 />
 </div>
 </div>
 ...
 className="w-full flex items-center gap-2.5 px-3 py-2.5 hover:bg-muted transition-colors text-left group border-b border-border"
 >
 <div className="w-6 h-6 rounded-lg bg-amber-500/15 border border-amber-500/25 flex items-center justify-center shrink-0">
 <PenLine className="w-3 h-3 text-muted-foregroundmber-400" />
 </div>
 <div>
 <p className="text-xs font-semibold text-muted-foregroundmber-300">Use "{query.trim()}"</p>
 <p className="text-[9px] text-muted-foreground">One-off · won't be saved to Clients</p>
 </div>
 </button>
 )}
 ...
 {/* Client list */}
 {!compact && !query && (
 <p className="px-3 pt-2.5 pb-1 text-[9px] uppercase tracking-wider font-bold text-muted-foreground">Existing Clients</p>
 )}

 {filteredClients.length === 0 && !query.trim() ? (
 <div className="px-3 py-4 text-xs text-muted-foreground text-center">No clients found. Add some in the Clients section.</div>
 ...
 >
 <div className="w-6 h-6 rounded-lg bg-gradient-to-br from-violet-500/20 to-blue-500/20 border border-border flex items-center justify-center text-[10px] font-bold text-foreground shrink-0">
 ...
 <kbd className="text-[9px] bg-muted border border-border rounded px-1 py-0.5 text-muted-foreground">Enter</kbd>
 <span className="text-[9px] text-muted-foreground">to use as custom client</span>
 </div>
 </motion.div>
 )}
 </AnimatePresence>
