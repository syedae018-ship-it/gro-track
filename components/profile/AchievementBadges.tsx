      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        {unlocked.slice(0, 5).map(a => (
          <span
            ...
            className="w-7 h-7 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-base cursor-default hover:scale-110 transition-transform"
          >
            {a.icon}
          </span>
        ))}
        {unlocked.length > 5 && (
          <span className="text-[10px] text-white/30 font-medium">+{unlocked.length - 5}</span>
        )}
      </div>
    )
  }
        ...
        return (
          <motion.div
            ...
            className={`relative rounded-2xl p-4 flex flex-col items-center text-center gap-2 border transition-all ${
              isUnlocked
                ? "bg-violet-500/5 border-violet-500/20 hover:border-violet-500/40 hover:bg-violet-500/10 hover:shadow-[0_0_20px_rgba(139,92,246,0.1)]"
          ...
          >
            ...
            <div>
              ...
              <p className="text-[10px] text-white/40 mt-0.5 leading-tight">{ach.description}</p>
            </div>
            ...
            {!isUnlocked && (
              <Lock className="w-3 h-3 text-white/20 absolute top-2 right-2" />
            )}
          </motion.div>
        )
      })}
