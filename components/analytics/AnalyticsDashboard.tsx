...
export const AnalyticsDashboard = React.memo(function AnalyticsDashboard({ employees, tasks, payouts }: Props) {
  ...
  }, [selectedEmployee, tasks, payouts, filterRange])
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ...
          { label: "Paid Out", value: formatINR(safeNum(agencyStats.totalPaid)), color: "text-primary", icon: TrendingUp, border: "border-primary/20", bg: "bg-primary/5" },
          { label: "Total Tasks", value: agencyStats.totalTasks, color: "text-white", icon: BarChart2, border: "border-white/10", bg: "bg-white/5" },
          ...
          <div key={s.label} className={`${s.bg} border ${s.border} rounded-xl p-4 flex flex-col gap-1.5`}>
            ...
            <span className="text-[9px] uppercase tracking-wider font-bold text-white/30">{s.label}</span>
          </div>
        ))}
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {employeeSummaries.map((emp, i) => (
            <motion.button
              ...
              className="text-left glass-card hover:border-primary/50 hover:shadow-purple-bloom p-4 group transition-all duration-300"
            >
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-white/40">{ROLE_LABELS[emp.role] || emp.role}</p>
                </div>
              ...
              <div className="flex items-center gap-2">
                <div className="flex-1 h-1.5 rounded-full bg-white/5 overflow-hidden">
                  <div
                    className={`h-full rounded-full bg-gradient-to-r ${emp.gradient}`}
                    style={{ width: `${emp.productivity}%`, transition: "width 1s ease" }}
                  />
                </div>
                <span className="text-[10px] text-white/40 font-semibold">{emp.productivity}%</span>
              </div>
            </motion.button>
          ))}
        </div>
              ...
              <div className="p-6 flex flex-col gap-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[
                    ...
                    <div key={k.label} className="bg-white/[0.03] border border-white/5 rounded-2xl p-4">
                      <span className="text-[9px] uppercase tracking-wider text-white/30 font-bold block mb-1">{k.label}</span>
                      <span className={`text-lg font-black ${k.color}`}>{k.value}</span>
                      <p className="text-[9px] text-white/20 mt-0.5">{k.sub}</p>
                    </div>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[
                    ...
                    <div key={m.label} className="bg-white/[0.03] border border-white/5 rounded-2xl p-4 flex flex-col gap-2">
                      <m.icon className={`w-4 h-4 ${m.color}`} />
                      <span className={`text-xl font-black ${m.color}`}>{m.value}</span>
                      <span className="text-[9px] uppercase tracking-wider text-white/30 font-bold">{m.label}</span>
                    </div>
                  ))}
                </div>
                    ...
                    {empDetail.monthlyEarnings.length > 0 ? (
                      <ResponsiveContainer width="100%" height={160}>
                        <LineChart data={empDetail.monthlyEarnings}>
                          ...
                          <Line type="monotone" dataKey="value" stroke="#8b5cf6" strokeWidth={2.5} dot={{ fill: "#8b5cf6", r: 3, strokeWidth: 0 }} activeDot={{ r: 5, fill: "#a78bfa" }} />
                        </LineChart>
                      </ResponsiveContainer>
                    ) : (
                      <div className="h-40 flex items-center justify-center text-xs text-white/20">No earnings data yet</div>
                    )}
                  </div>
                          ...
                          {empDetail.clientPie.map((c: any, idx: number) => (
                            <div key={idx} className="flex items-center justify-between gap-2">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-[11px] text-white/70 truncate max-w-[90px]">{c.name}</span>
                              </div>
                  ...
                  <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-5 flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-3">
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-white/40">Productivity Score</span>
                        <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-white/40">Revision Count</span>
                        <span className={`text-xs font-bold ${empDetail.revisionCount > 0 ? "text-amber-400" : "text-white/40"}`}>
